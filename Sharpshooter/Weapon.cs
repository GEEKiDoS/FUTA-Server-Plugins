...
namespace Sharpshooter
{
    public class Weapon
    {
        ...
        private static string[] _killStreak1List = new string[] { "ac130_105mm", "ac130_40mm", "ac130_25mm", "uav_strike_marker" };
        private static string[] _killStreak2List = new string[] { "nuke" };
        ...
        private WeaponType GetWeaponType(string baseWeapon)
        {
            ...
            else if (_killStreak1List.Contains(baseWeapon))
            {
                return WeaponType.KillstreakHandheld;
            }
            else if (_killStreak2List.Contains(baseWeapon))
            {
        ...
        public static Weapon QueryWeapon(string fullcode)
        {
            Weapon weapon = new Weapon();

            var split = fullcode.Split(new char[] { '_' });
            var len = split.Length;

            int attindex;
            if (split[0] == "iw5")
            {
                weapon.BaseName = "iw5_" + split[1];
                attindex = 3;
            }
            else
            {
            ...
            for (int i = attindex; i < split.Length; i++)
            {
                if (split[i].Contains("camo"))
                {
                    weapon.Attachment1 = "none";
                    weapon.Attachment2 = "none";
                    weapon.Camo = Convert.ToInt32(split[i].Substring(4));
                    weapon.Reticle = i == split.Length - 2 ? Convert.ToInt32(split[i + 1].Substring(7)) : 0;
                    break;
                }
                else
                {
                    ...
                    switch (weapon.Type)
                    {
                        case WeaponType.AR:
                            ...
                            if (GetAr1Attachments1.Contains(split[i]) || GetAr1Attachments2.Contains(split[i]))
                            {
                                att = split[i];
                            }
                            else if (GetAr2Attachments1.Contains(split[i]) || GetAr2Attachments2.Contains(split[i]))
                            {
                                att = split[i];
                            }
                            else if (GetAr3Attachments1.Contains(split[i]) || GetAr3Attachments2.Contains(split[i]))
                            {
                                att = split[i];
                            }
                            break;
                        case WeaponType.SMG:
                            split[i].Replace("smg", "");
                            if (GetSmgAttachments1.Contains(split[i]) || GetSmgAttachments2.Contains(split[i]))
                            {
                                att = split[i];
                            }
                            break;
                        case WeaponType.LMG:
                            split[i].Replace("lmg", "");
                            if (GetLmgAttachments1.Contains(split[i]) || GetLmgAttachments2.Contains(split[i]))
                            {
                                att = split[i];
                            }
                            break;
                    ...
                    if (i == attindex)
                    {
                        weapon.Attachment1 = att;
                    }
                    else if (i == attindex + 1)
                    {
                        weapon.Attachment2 = att;
                    }
                }
            }
        ...
        public static Weapon GetWeaponByName(string basename)
        {
            if (basename.StartsWith("iw5_"))
            {
                return QueryWeapon(basename + "_mp");
            }
            else
            {
                if (basename == "pkp")
                {
                    return QueryWeapon("iw5_pecheneg_mp");
                }
                else if (basename == "l86")
                {
                    return QueryWeapon("iw5_sa80_mp");
                }
                else if (basename == "l118a")
                {
                    return QueryWeapon("iw5_l96a1_mp");
                }
                else if (basename == "pm9")
                {
                    return QueryWeapon("iw5_m9_mp");
                }
                else if (basename == "svd")
                {
                    return QueryWeapon("iw5_dragunov_mp");
                }
                else if (basename == "m82")
                {
                    return QueryWeapon("iw5_barrett_mp");
                }
                else if (basename == "nz3")
                {
                    return QueryWeapon("uav_strike_marker_mp");
                }
                else if (basename == "riot")
                {
                    return QueryWeapon("riotshield_mp");
                }
                else
                {
                    return QueryWeapon(basename + "_mp");
